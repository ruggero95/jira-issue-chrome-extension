...
import { getProjectUIUrl } from "../api/jira.api"
...
export const CardProject: React.FC<{ htmlId: string, project: Project, inputName: string }> = ({ htmlId, project, inputName }) => {
    ...
    return (
        <Card className="cursor-pointer h-[110px]" onClick={() => setSettings({ ...settings, project: id, projectKey: key })} key={`project-${htmlId}`}>
